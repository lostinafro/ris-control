...
from environment import ecdf, OUTPUT_DIR, TAU, BANDWIDTH
...
axes_kpi.plot(TAU, kpi_fixed, linewidth=1.5, linestyle='--', label=f'CB-BSW: Fixed', color='blue')
axes_kpi.plot(TAU, kpi_flexi, linewidth=1.5, linestyle=':', label=f'CB-BSW: Flexible', color='red')
...
for ss, setup in enumerate(setups):
    ...
    for pp, paradigm in enumerate(paradigms):
        ...
        ## RATE (FAKE)
        # Load data
        rate = BANDWIDTH * np.load(datafilename)['rate_real'] / 1e6

        mean_rate_vs_tau =  np.mean(rate, axis=1)
        axes_me[ss].plot(TAU, mean_rate_vs_tau, linewidth=1.5, linestyle=styles[pp], label=labels[pp], color=colors[pp])
